...
def find_curators(messages):
    ...
    for m in messages:
        ...
        if uid in candidate_ids:
            display = m.get("from", "")
            if not username or username in display.lower() or username in flatten_text(m).lower():
...
if __name__ == "__main__":
    ...
    if out:
        ...
        pprint.pp(qas[:2])  # печатаем первые 2 для проверки
    ...
    if out:
        Path(out).write_text(json.dumps(qa, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"Saved {len(qa)} Q&A pairs → {out}")
    else:
